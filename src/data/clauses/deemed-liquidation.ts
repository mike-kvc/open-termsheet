...
Deemed Liquidation이 트리거되었을 때 매각가가 총 투자금보다 낮은 경우(이른바 'down exit')를 구체적인 수치로 살펴보겠습니다. 총 투자금이 50억 원인데 회사가 40억 원에 매각되는 경우, Non-participating 1x 기준으로 투자자가 40억 원 전액을 우선 회수하게 됩니다. 이 경우 창업자 몫은 0원입니다. 수년간 회사를 운영하고도 M&A에서 한 푼도 받지 못하는 상황이 발생하는 것입니다. Participating 구조라면 더욱 극단적으로, 매각가가 투자금을 초과하더라도 투자자가 우선 회수 + 잔여 배분 참여로 대부분을 가져가므로, 창업자의 실질 수령액이 예상보다 크게 낮아집니다.
...
트리거 조건 설계가 핵심입니다. '일련의 거래' 합산 조항과 특수관계인 취득분 합산 조항을 반드시 포함해야 분할 매수를 통한 우회를 막을 수 있습니다. 경영권 이전 기준(보통 의결권 50% 초과)이 분할 매수로 우회될 수 있는 위험에 특히 주의해야 합니다. 예를 들어, 전략적 인수자가 먼저 49%를 매수하고 6개월 뒤 추가 10%를 매수하는 경우, '일련의 거래(series of transactions)' 합산 조항이 없으면 두 번의 거래 모두 개별적으로 50% 미만이므로 트리거가 발동하지 않습니다. 또한 특수관계인(자회사, 계열사, 특수목적법인 등)을 통해 분산 매수하는 방식도 우회 수단으로 사용될 수 있습니다. 따라서 '일련의 관련 거래를 합산하여 판단한다'는 조항과 '특수관계인의 취득분을 합산하여 판단한다'는 조항이 반드시 포함되어야 합니다.`,
...
- Participating 여부와 연동하여 협상 — Deemed Liquidation이 포괄적이라면 Non-participating으로 조정하는 딜도 가능`,
  ...
  war_stories: [
    {
      title: "대주주 지분 분할 매수로 Deemed Liquidation 우회",
      situation:
        "대주주(60%)가 전략적 투자자에게 본인 지분을 매각했는데, 매수자가 여러 명의 특수관계인 명의로 나눠서 각각 15%씩 매수. 개별 거래로는 계약서의 경영권 이전 기준(50% 이상)을 넘지 않았습니다.",
      consequence:
        "Deemed Liquidation이 트리거되지 않아 잔여재산 우선분배를 주장할 수 없었습니다. 결과적으로 실질적 경영권은 이전되었는데 투자자의 보호 장치는 작동하지 않았습니다.",
      lesson:
        "Deemed Liquidation의 경영권 이전 정의에 '일련의 거래(series of transactions) 합산'과 '특수관계인 취득분 합산' 조항이 없으면 분할 매수로 우회됩니다.",
    },
  ],
};
