...
다운라운드가 실제로 발생하는 주요 원인은 시장 환경 악화(경기 침체, 금리 인상 등), 회사의 매출·KPI 목표 미달성, 경쟁사의 부상으로 인한 시장 지위 약화 등입니다. 다운라운드 상황에서 창업자가 취할 수 있는 전략적 대응 중 하나는 pay-to-play 조항을 제안하는 것입니다. 이는 '후속 라운드에 프로라타(pro-rata) 이상으로 참여하지 않는 기존 투자자는 anti-dilution 보호를 상실한다'는 조건으로, 참여하지 않는 투자자의 우선주를 보통주로 전환시켜 anti-dilution 보호를 해제시키는 효과가 있습니다. 이를 통해 다운라운드의 희석 부담을 실제로 참여하는 투자자와 창업자만이 분담하는 구조를 만들 수 있습니다.`,
...
한국 시장에서 가중평균이 표준인 데에는 구조적인 이유가 있습니다. KVIC(한국벤처투자) 모태펀드 출자 시 full ratchet을 지양하는 가이드라인이 존재하며, 모태펀드 출자를 받은 대부분의 VC는 이 기준을 따릅니다. 한국 VC 시장에서 모태펀드의 비중이 상당하기 때문에 이 가이드라인이 시장 전체의 표준을 형성하는 효과가 있습니다. 실무에서 다운라운드가 발생하면 기존 투자자와 신규 투자자 사이에 긴장이 발생합니다. 기존 투자자는 anti-dilution 조정으로 전환주식수가 늘어나는데, 이는 신규 투자자 입장에서 자신의 지분을 희석시키는 효과가 있어 딜 구조 협상이 복잡해집니다. 신규 투자자가 기존 투자자의 anti-dilution 포기(waiver)를 조건으로 투자를 진행하는 경우도 있습니다.`,
...
- 주식분할, 무상증자, 스톡옵션 행사 등에 의한 희석과는 별도의 조정 메커니즘이 적용됨에 유의`,
  ...
  legal_check: {
    applicable_laws: [
      ...
      {
        name: "벤처투자조합 규약 및 LP 보고 기준",
        relevance:
          "벤처투자조합이 투자한 경우, 전환가격 조정(Anti-dilution 적용) 등 투자 조건 변경 시 LP 보고 또는 내부 승인 의무가 발생할 수 있습니다. 다운라운드로 인한 전환가격 조정 내역은 조합 규약 및 관련 규정에 따라 조합원(LP)에게 적시에 통지되어야 합니다.",
      },
      {
        name: "자본시장법 (상장 전환 시)",
        relevance:
  ...
  war_stories: [
    {
      title: "70% 다운라운드 + Full Ratchet → 창업자 지분 25%로 급락",
      situation:
        "시리즈A에서 주당 10,000원에 30억을 투자받았습니다(지분 30%). Full ratchet anti-dilution 조항이 포함되어 있었는데, 당시에는 '어차피 업라운드만 하면 되니까'라고 생각했습니다. 1년 후 시장 침체로 시리즈B가 주당 3,000원(70% 다운)에 진행되었습니다.",
      consequence:
        "시리즈A 투자자의 전환가격이 10,000원 → 3,000원으로 조정. 전환주식수가 3.3배로 증가. 창업자 지분이 70%에서 약 35%로 급락. 시리즈B 투자자 유치도 어려워졌는데, 새 투자자 입장에서 full ratchet이 있는 기존 투자 구조가 부담이었기 때문입니다.",
      lesson:
        "Full ratchet은 '어차피 다운라운드 안 하면 되지'라고 생각하기 쉽지만, 시장 환경은 창업자가 통제할 수 없습니다. 가중평균(weighted average)이 한국 시장의 표준인 데는 이유가 있습니다.",
    },
  ],
};
